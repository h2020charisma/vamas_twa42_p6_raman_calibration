...
*Status: 2026-07-07, branch `troubleshoot/xcal` (pipeline) + `issue233` (ramanchada2).
...
| §6.2.3–6.2.4 Sections 3–4 — spectral distribution, pixel/spectral resolution, SpeD:SRes curves | `spectrares_[[key]]` (`spectraframe_resolution.py`): Ne FWHM vs position (Gaussian, NIST-line-matched), calcite ~1085.91 cm⁻¹ FWHM → ASTM E2529 spectral resolution, laser-effect-scaled resolution curve, SpeD:SRes ratio; `resolution_compare` aggregates per-laser-wavelength across instruments and derives a harmonization-target resolution envelope | ✅ |
...
| §8 Calibration files (portable point list + spline, Si position, calibrated laser wl) | Models persisted as **portable JSON** (`calmodel_*.json`, point list + Si position + calibrated laser wl, regenerable without pickle) alongside the legacy pickle (`calmodel_*.pkl`, still authoritative for the running pipeline) | ✅ (JSON complementary, not yet the pipeline's load path) |
...
8. **Mandate a portable, language-neutral calibration file format in §8.** As written, §8 lists
   the required *content* (point list, Si position, calibrated laser wavelength) but not a
   format, which invites implementations to reach for a language-specific serialisation (e.g.
   Python pickle) that other tools and future code versions cannot reliably read back. We
   validated the alternative ourselves: exporting the same content as plain JSON (+ CSV point
   list) is sufficient to regenerate the calibration spline with no dependency on pickle or a
   specific codebase. Recommend the CWA require an open, self-describing format (JSON/CSV or
   NeXus/HDF5) for interchange, even if implementations keep a faster native cache internally.
